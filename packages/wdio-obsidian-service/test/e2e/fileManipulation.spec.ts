...
describe("Test page object", () => {
    ...
    it("delete folder", async () => {
        ...
        const {folders} = await browser.executeObsidian(({app}) => app.vault.adapter.list("/"));
        expect(folders.sort()).toEqual(['.obsidian']);

        // delete a second time does nothing
        await obsidianPage.delete("B");
    })

    it("delete hidden folder", async () => {
