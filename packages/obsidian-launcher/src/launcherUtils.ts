...
export async function checkIfAppAndInstallerAreCompatible(
    ...
    if (appVersion == installerVersion) {
    ...
    try {
        ...
        if (debugInfoResult.result) {
            const debugInfo: string = await until(
                () => cdpEvaluate(client, 'document.querySelector(".debug-textarea").value'),
                5000,
            );
...
export async function populateMinInstallerVersion(
    ...
    try {
        ...
        // copy versions, drop the versions before the first installer.
        const versionArr = _(_.cloneDeep(versions))
            .sort((a, b) => semver.compare(a.version, b.version))
            .dropWhile(v => !!v.downloads.appImage)
            .value();
        ...
        const alreadySet = new Set(versions
            .filter(v => v.minInstallerVersion && v.maxInstallerVersion)
            .map(v => v.version)
        );
        return versionArr
            .filter(v => !alreadySet.has(v.version))
            .map(v => ({
                version: v.version,
